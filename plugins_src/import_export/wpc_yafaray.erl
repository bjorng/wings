...
%%% Transmit Filter also for Glass and Rough Glass
   ...
   Transparency =
        proplists:get_value(transparency, YafaRay,
                        ?DEF_TRANSPARENCY),
   TransmitFilter =
        proplists:get_value(transmit_filter, YafaRay,
                        ?DEF_TRANSMIT_FILTER),
   Translucency =
        proplists:get_value(translucency, YafaRay,
                        ?DEF_TRANSLUCENCY),
   SpecularReflect =
        proplists:get_value(specular_reflect, YafaRay,
                        ?DEF_SPECULAR_REFLECT),	                    
   Emit =
        proplists:get_value(emit, YafaRay,
                        ?DEF_EMIT),
...
%%%
   ...
   ScatterColor =
        proplists:get_value(scatter_color, YafaRay,
	                    ?DEF_SCATTER_COLOR),
   SigmaSfactor =
        proplists:get_value(sigmas_factor, YafaRay,
	                    ?DEF_SIGMAS_FACTOR),
   SSS_Translucency =
   	proplists:get_value(sss_translucency, YafaRay,
   				?DEF_SSS_TRANSLUCENCY),
   SSS_Specular_Color =
        proplists:get_value(sss_specular_color, YafaRay,
	                    ?DEF_SSS_SPECULAR_COLOR),
...
%%%
  ...
  DiffuseReflect =
        proplists:get_value(diffuse_reflect, YafaRay,
	                    ?DEF_DIFFUSE_REFLECT),
  OrenNayar =
  	proplists:get_value(oren_nayar, YafaRay,
  			?DEF_OREN_NAYAR),

  OrenNayar_Sigma =
  	proplists:get_value(oren_nayar_sigma, YafaRay,
...
%%%
  ...
  GlossyReflect =
        proplists:get_value(glossy_reflect, YafaRay,
	                    ?DEF_GLOSSY_REFLECT),
  Exponent =
        proplists:get_value(exponent, YafaRay,
	                    ?DEF_EXPONENT),
  Anisotropic =
  	proplists:get_value(anisotropic, YafaRay,
  			?DEF_ANISOTROPIC),
  Anisotropic_U =
  	proplists:get_value(anisotropic_u, YafaRay,
  			?DEF_ANISOTROPIC_U),
  Anisotropic_V =
  	proplists:get_value(anisotropic_v, YafaRay,
  			?DEF_ANISOTROPIC_V),
...
%%%
   ...
   Lightmat_Power =
        proplists:get_value(lightmat_power, YafaRay,
	                    ?DEF_LIGHTMAT_POWER),
...
%%%
   ...
   Blend_Mat1 =
        proplists:get_value(blend_mat1, YafaRay,
	                    ?DEF_BLEND_MAT1),

   Blend_Mat2 =
        proplists:get_value(blend_mat2, YafaRay,
	                    ?DEF_BLEND_MAT2),

   Blend_Value =
   	proplists:get_value(blend_value, YafaRay,
   			?DEF_BLEND_VALUE),
...
{hframe,[
 ...
 {hframe,
	[{menu,[{?__(82,"Uniform"),uniformvolume},{?__(83,"ExpDensity"),expdensityvolume},{?__(126,"Noise"),noisevolume}],
	  Volume_Type,
	  [key(volume_type),layout]},
...
%%% End Noise Volume	    
	...
	[{slider, {color, Meshlight_Color, [key(meshlight_color)]}}
			...
			{vframe, [{button,"Set Default",keep,
			     [diffuse_hook(?KEY(meshlight_color))]},
...
%%%
	...
	{vframe,
	 [{menu,
	   [{?__(9,"Shiny Diffuse"),shinydiffuse},{?__(29,"Glass"),glass},{"Rough Glass",rough_glass},{"Glossy",glossy},
	   {"Coated Glossy",coatedglossy},{"Translucent (SSS)",translucent},{"Light Material",lightmat},{"Blend",blend_mat}],
	   ShaderType,
...
%%%
			...
			{label, "Reflected Color"}, {label, "Diffuse Color"},{label, "Transparency"},{label, "Transmit Filter"},
			{label, "Translucency"},{label, "Diffuse Reflection"},{label, "Mirror Reflection"},{label, "Emit Light"},
			{"Oren-Nayar",OrenNayar,[key(oren_nayar)]},{"Fresnel Effect",TIR,[key(tir)]},panel
			]},
			{vframe, [
				...
				{slider, {color, Transmitted, [key(transmitted)]}},
               		    ...
               		    {hframe,[{label, "Sigma"},{text,OrenNayar_Sigma,[range(oren_nayar_sigma), key(oren_nayar_sigma),
                		            hook(enable, ['not',[member,?KEY(oren_nayar),
					    ?DEF_OREN_NAYAR]])]}
					    ]}	                                
                                
				]},
					    
			{vframe, [panel,panel,{button,"Set Default",keep,
			     [diffuse_hook(?KEY(transmitted))]}]}
...
%%%
	    	...
	    	{hframe, [
			{vframe, [{label, "Index of Refraction"},{label, "Internal Reflection"},
			{label, "Reflected Light"}, {label, "Filtered Light"},{label, "Absorption Color"},{label, "Absorption Distance"},
			{label, "Transmit Filter"},{label, "Dispersion Power"},{label, "Dispersion Samples"},{"Fake Shadows",FakeShadows,[key(fake_shadows)]},panel]},
			{vframe, [
                                {slider, {text, IOR,[range(ior),{width,5}, key(ior)]}},
			...
			{vframe, [panel,panel,panel,{button,"Set Default",keep,[transmitted_hook(?KEY(transmitted))]},
...
%%%
			...
			{vframe, [{label, "Index of Refraction"},
			{label, "Reflected Light"}, {label, "Filtered Light"},{label, "Absorption Color"},{label, "Absorption Distance"},
			{label, "Transmit Filter"},{label, "Roughness"},{label, "Dispersion Power"},{label, "Dispersion Samples"},
			...
			{vframe, [panel,panel,{button,"Set Default",keep,[transmitted_hook(?KEY(transmitted))]}
...
%%%
	    	...
	    	{hframe, [
			{vframe, [{label, "Glossy Color"}, {label, "Diffuse Color"},
			...
			{vframe, [panel,{button,"Set Default",keep,
			     [diffuse_hook(?KEY(transmitted))]}]}
...
%%%
			...
			{label, "Diffuse Reflection"},{label, "Glossy Reflection"},
			{label, "Exponent"},{"Anisotropic",Anisotropic,[key(anisotropic)]},
			{"Oren-Nayar",OrenNayar,[key(oren_nayar)]},		
			panel
			]},
			{vframe, [
				...
				{slider, {color, Transmitted, [key(transmitted)]}},
                                ...
                                {slider, {text,GlossyReflect,[range(glossy_reflect), key(glossy_reflect)]}},
                                {slider, {text,Exponent,[range(exponent),{width,8}, key(exponent),
                 		            hook(enable, [member,?KEY(anisotropic),
					    ?DEF_ANISOTROPIC])]                               
                                }},

                             
               		    {hframe,[{label, "Exp U"},    {text,Anisotropic_U,[range(anisotropic_u), key(anisotropic_u),
                		            hook(enable, ['not',[member,?KEY(anisotropic),
					    ?DEF_ANISOTROPIC]])]},

               		    {label, "Exp V"},    {text,Anisotropic_V,[range(anisotropic_v), key(anisotropic_v),
                		            hook(enable, ['not',[member,?KEY(anisotropic),
					    ?DEF_ANISOTROPIC]])]}]},
...
%%%
	    	...
	    	{hframe, [
			{vframe, [{label, "Index of Refraction"},
			{label, "Glossy Color"}, {label, "Diffuse Color"},{label, "Specular Color"},
			{label, "Absorption Color"},{label, "Absorption Distance"},{label, "Scatter Color"},{label, "SigmaSfactor"},
			{label, "Diffuse Reflection"},{label, "Glossy Reflection"},{label, "Translucency"},
			{label, "Exponent"},
			panel
			]},
			{vframe, [
...
%%%
			...
			{vframe, [{button,"Set Default",keep,
			     [diffuse_hook(?KEY(lightmat_color))]}]}
...
%%%
	    	...
	    	{hframe, [
			{vframe, [{label, "Material 1"},{label, "Material 2"},{label, "Blend Mix"},
			panel
			]},
			{vframe, [
